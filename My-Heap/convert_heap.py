class HeapConverter(object):
    ...
    def __init__(self, heap_space):
        # represent the My-Queue with a one-dimensional array
        self.heap = heap_space

    def transform(self):

        # we do not have to consider leaf nodes (that's why the last node we have to consider has indexed (length-2)/2
        index = (len(self.heap) - 2) // 2

        # consider the internal node in a reverse order
        while index >= 0:
    ...
    # argument is the index of an internal node in the My-Queue
